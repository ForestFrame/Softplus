...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnSoftplusGetWorkspaceSize(inputTensor_[0], outputTensor_[0], &workspaceSize, &handle);  // ! 有几个输入几个输出就写几个
    if (ret != ACL_SUCCESS) {
    ...
    {
        Timer timer;
        if (aclnnSoftplus(workspace, workspaceSize, handle, stream) != ACL_SUCCESS) {
            (void)aclrtDestroyStream(stream);  // !
